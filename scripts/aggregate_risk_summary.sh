...
derive_problem() {
  case "$1" in
    ...
    CICD-SEC-07*|CICD-SEC-05-07*) echo "Runner isolation and hardening controls are insufficient for sensitive workloads." ;;
...
derive_exploit_path() {
  case "$1" in
    ...
    CICD-SEC-07*|CICD-SEC-05-07*) echo "Compromised jobs can pivot through weak runner controls into the host or adjacent workloads." ;;
...
derive_impact() {
  case "$1" in
    ...
    CICD-SEC-07*|CICD-SEC-05-07*) echo "Runner compromise can lead to lateral movement and persistent CI/CD backdoors." ;;
